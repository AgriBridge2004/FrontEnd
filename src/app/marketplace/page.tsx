...
import type { Language, Product as DetailProduct } from "@/types/types";
      ...
      <div className="min-h-screen bg-[#fafdfa] flex flex-col selection:bg-brand-100 selection:text-brand-900" dir={isRtl ? 'rtl' : 'ltr'}>
        ...
        <main className="flex-grow">
          <ProductDetails 
            ...
            onSelectProduct={(id) => {
              const richProd = RICH_PRODUCTS.find((p) => p.id === id);
              if (richProd) {
            ...
            {PRODUCTS.map((p) => (
              <ProductCardComponent 
                ...
                onViewDetails={(product) => {
                  const detailProduct = mapToDetailProduct(product);
                  if (detailProduct) {
