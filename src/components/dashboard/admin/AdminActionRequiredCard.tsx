...
export function AdminActionRequiredCard({ onDisputeClick, onViewAllDisputes }: AdminActionRequiredCardProps) {
  ...
  return (
    <DashboardCard className="overflow-hidden p-4">
      <h2 className="text-base font-black text-slate-950">Action Required</h2>

      <div className="mt-5 grid grid-cols-2 border-b border-slate-100 text-center">
      ...
      <div className="mt-3 grid gap-2.5">
        {activeTab === "disputes" ? (
          openDisputes.map((item) => (
            <button
              className="grid grid-cols-[32px_minmax(0,1fr)_24px] items-center gap-2.5 rounded-xl p-2 text-left transition-all duration-200 hover:border-emerald-200 hover:bg-emerald-50/30 hover:shadow-md"
              key={item.id}
              onClick={onDisputeClick}
              type="button"
            >
              <span className="grid size-7 place-items-center rounded-lg bg-orange-50 text-orange-600">
                <AlertTriangle className="size-3.5" />
              </span>
              <span className="min-w-0">
                <span className="flex items-start justify-between gap-2">
                  <span className="text-[13px] font-black leading-4 text-slate-950">{item.title}</span>
                  <span className="shrink-0 text-[11px] font-black text-red-600">{item.time}</span>
                </span>
                <span className="mt-1 grid grid-cols-2 gap-2 text-[10px] font-medium leading-4 text-slate-500">
...
function ActionTabButton({ active, label, onClick }: { active: boolean; label: string; onClick: () => void }) {
  return (
    <button
      className={cn(
        "relative h-10 px-2 text-[13px] font-black transition",
