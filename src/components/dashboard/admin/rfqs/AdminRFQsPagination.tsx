...
export function AdminRFQsPagination({ currentPage, onPageChange, onPageSizeChange, pageSize, totalPages }: AdminRFQsPaginationProps) {
  return (
    <div className="flex flex-col gap-3 border-t border-slate-100 px-4 py-4 text-[12px] font-semibold text-slate-500 sm:flex-row sm:items-center sm:justify-between">
      ...
      <nav aria-label="RFQs pagination" className="flex items-center overflow-hidden rounded-lg border border-slate-200 bg-white">
  ...
  return (
    <button
      className={cn(
        ...
        isActive ? "bg-emerald-800 text-white" : "bg-white text-slate-600 hover:bg-emerald-50/40 hover:text-emerald-800",
