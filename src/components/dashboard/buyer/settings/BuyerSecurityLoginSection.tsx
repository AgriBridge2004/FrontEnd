...
import { BuyerSettingsCard, BuyerToggle } from "@/components/dashboard/buyer/settings/BuyerSettingsControls";
import type { BuyerSession } from "@/components/dashboard/buyer/settings/buyer-settings.types";

type BuyerSecurityLoginSectionProps = {
...
export function BuyerSecurityLoginSection({
  onChangePassword,
  onRevokeSession,
  onToggleTwoFactor,
  sessions,
  twoFactorEnabled,
}: BuyerSecurityLoginSectionProps) {
  return (
    <BuyerSettingsCard id="security-login" title="Security & Login">
      <button
      ...
      <div className="mt-4 flex items-center justify-between gap-4 rounded-xl border border-emerald-100 bg-white p-3">
        ...
        <BuyerToggle checked={twoFactorEnabled} onChange={onToggleTwoFactor} />
      </div>
