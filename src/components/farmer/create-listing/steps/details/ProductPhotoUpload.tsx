...
import type { UploadPhotoOption } from "@/components/farmer/create-listing/create-listing.types";
...
export function ProductPhotoUpload({ photos, onPhotosChange }: ProductPhotoUploadProps) {
  const [photoSlots, setPhotoSlots] = useState<ProductPhotoSlot[]>(photos);
  ...
  function updatePhotoSlot(index: number, event: ChangeEvent<HTMLInputElement>) {
    ...
    setPhotoSlots((currentSlots) => {
      ...
      const nextPreviewUrls = nextSlots.flatMap((slot) => (slot.previewUrl ? [slot.previewUrl] : []));
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3">
      {photoSlots.map((photo, index) => {
        ...
        return (
          <div className="relative" key={photo.label}>
            <input
              accept="image/*"
            ...
            <button
              aria-label={`Upload product image ${index + 1}`}
              className="group relative h-32 w-full cursor-pointer overflow-hidden rounded-xl border border-dashed border-emerald-200 bg-emerald-50 text-slate-900 shadow-sm transition-all duration-200 hover:border-emerald-400 hover:shadow-md"
            ...
            >
              {hasUploadedImage && photo.previewUrl ? (
                ...
                <Image
                  ...
                  className="object-cover blur-[1.5px] transition duration-200 group-hover:scale-105"
                  fill
